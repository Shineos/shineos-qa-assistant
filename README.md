...
| GPU自動活用 | **NVIDIA GPU があれば自動でGPU処理（高速）**。なければ CPU で動作（どちらもインストール時に自動判定） |
| 他アプリへの配慮 | AI応答中でも他のアプリを優先（CPU機のみ優先度制御・メモリ自動調整。GPU機はCPU負荷が小さいため速度最優先） |
...
- 空き容量 15GB 以上
- インストール時のみインターネット接続（Ollama本体1.5GB＋AIモデル約3.4GBなど、**合計約6GB**のダウンロード。ダウンロードは並列実行され、所要は回線により**約15〜40分**）
...
4. **資料作成**: 「/pdf 経費精算の手順について」「/pptx 〜」「/docx 〜」のように送ると、ナレッジから資料（PDF / PowerPoint / Word）を自動生成（約1〜2分）
...
インストーラの操作からインストール完了までの実際の流れです（3倍速）。

![インストールの流れ](assets/videos/installer.gif)

### 使用例（動画）

質問してから出典付きの回答が返るまでの実際の流れです（実際の操作を録画）。
...
| [終了コード一覧](docs/exit-codes.md) | インストーラ（Inno Setup 標準）と内部スクリプトの終了コード・無人インストールの挙動 |
