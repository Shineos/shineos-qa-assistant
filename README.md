...
| 図面PDF検索・Q&A（製造業向け拡張・設定から有効） | 図面PDFの表題欄（図番・品名・材質・改訂）を取り込み時に自動読み取り。図番の表記ゆれ（`A-1234`／`A1234`／全角）も同一視して検索できます。画面キャプチャ（`Win+Shift+S` → 貼り付け）で図番を読み取って質問も可能（OCRはPC内で処理・既定OFF） |
