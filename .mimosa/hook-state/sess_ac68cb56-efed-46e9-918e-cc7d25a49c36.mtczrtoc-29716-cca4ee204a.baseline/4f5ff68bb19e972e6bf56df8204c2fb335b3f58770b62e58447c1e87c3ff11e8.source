...
使い方（管理者権限で実行。実行前に ShineosQA サービスを停止すること）:
  ...
  2. venv の python で本スクリプトを実行
       "C:\\Program Files\\ShineosQA\\venv\\Scripts\\python.exe" purge_orphan_vectors.py --needles "KZ-,検証用文書"
...
KID = None  # None なら全コレクションを対象に走査


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--path', default=r'C:\Program Files\ShineosQA\data\vector_db')
    ap.add_argument('--kid', default=KID, help='対象コレクション（ナレッジid）。未指定なら全コレクション走査')
