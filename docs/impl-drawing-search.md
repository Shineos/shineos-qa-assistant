...
#### OCR前処理のアンサンブル実験（2026-09-23・試行→撤回）

- 試行: 各回転方向のOCRを「素の画像」「グレースケール＋コントラスト伸張（0.1%パーセンタイル）」の2候補で実行し、読み方向スコアで最良を採用する方式（`EnhanceForOcr`）
- 結果: 金賞作品ページ（gold-p1.png）でキーワード一致が4→2に**低下**（0403は維持、0419・普通公差を取りこぼし）。読み方向スコアは「文字量×語長²」のため、前処理で文字化けが長いトークンに化けても高スコアになり、内容品質を判定できない
- 判定: クリアな画像で悪化するため撤回。スキャン品質の本質対応は **VL文字起こし取り込み**（既存Qwen3-VLの転用・次期候補）で行う
- 残した改善: OCRフォールバックのレンダ解像度 2200→2600（エンジン上限）。金賞作品スキャンの取り込みテキストがチャンク1→2に増加を実測
