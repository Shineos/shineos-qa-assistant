...
- 実図面5問（`scenarios-cadreal.json` 新設）: **4/5**。図番特定・尺度・ねじ規格・不存在図番の拒否が合格。
  cr02（普通公差のJIS規格）は 0403-CT8 を正しく引用するも 0405-m の併記に届かず。
  チャンクテキストには 0405 が存在（プローブ確認済み）→ 1.7B模型の参照能力限界（既知限界として記録）
- 18問: 15/18（s05/s07 は本開発環境コーパスの言い回しが期待語と不一致のテスト環境差、s12 はmain既存の揺らぎ。
  本ブランチの変更は 図面/図番 質問でのみ発動するため、これら3問に因果経路はない）

### 残課題

- cr02型: 複数図面横断の引用完成度（模型上限。4B以上の階級での再確認を推奨）
