...
# ---- セキュリティ: 出力パスの安全化（LLM/ユーザー由来のファイル名対策 v1.0.60） ----
_FILENAME_SAFE = re.compile(r"[^\w\-.()\[\] ]+", re.UNICODE)


def _safe_filename(name, fallback="export"):
    """ユーザー/LLM由来のファイル名からディレクトリ移動（../ 等）と危険文字を除去する"""
    name = os.path.basename(str(name or ""))
    name = _FILENAME_SAFE.sub("_", name).strip("._ ")
    return name or fallback


def _resolve_output_path(folder_path, filename):
    """出力先がエクスポートフォルダの外へ出ないことを保証して絶対パスを返す"""
    base = os.path.abspath(folder_path)
    full = os.path.abspath(os.path.join(base, _safe_filename(filename)))
    if os.path.commonpath([base, full]) != base:
        raise ValueError("output path escapes export directory")
    return full
# ---- セキュリティここまで ----
...
def search_image(query):
    ...
    # オフライン設計のため既定はローカルSD（外部画像APIは廃止 v1.0.60）
    image_source = os.getenv("IMAGE_SOURCE", "local_sd")

    if image_source == "local_sd":
        return search_local_sd(query)
    else:
        log.warning(f"Image source unknown/offline : {image_source}")
        return None
...
def search_local_sd(query: str):
    ...
    try:
        ...
        image_b64 = images[0]
        # オフライン設計: 生成画像はローカルファイルサーバー経由ではなく
        # バイナリで直接返す（動的URL取得のSSRF対策 v1.0.60）
...
def search_unsplash(query):
    # オフライン設計のため外部画像APIは使用しない（SSRF・プライバシー対策 v1.0.60）
    log.warning("External image search is disabled in the offline build.")
    return None
                    ...
                    for img_tag in imgs:
                        ...
                        try:
                            if src and src.startswith("data:image"):
                                img_data = BytesIO(base64.b64decode(src.split(",", 1)[1]))
                                img = Image(img_data, width=200, height=150)
                            elif src and src.startswith("http"):
                                # オフライン設計: リモート画像は取得しない（SSRF対策 v1.0.60）
                                log.debug(f"Skipping remote image (offline): {src}")
                                img = None
                            else:
                ...
                if src is not None: 
                    try:
                        if src.startswith("image_query:"):
                            ...
                            image_data = search_image(query)
                            if image_data:
                                img = ReportLabImage(BytesIO(image_data), width=200, height=150)
                                story.append(img)
                                story.append(Spacer(1, 10))
                            else:
                            ...
                            img = ReportLabImage(BytesIO(base64.b64decode(src.split(",", 1)[1])), width=200, height=150)
                            story.append(img)
                            story.append(Spacer(1, 10))
                        elif src.startswith("http"):
                            # オフライン設計: リモート画像は取得しない（SSRF対策 v1.0.60）
                            log.debug(f"Skipping remote image (offline): {src}")
                            story.append(Paragraph("[Image: remote images are disabled in offline build]", styles["CustomNormal"]))
                            story.append(Spacer(1, 6))
                        else:
                            log.debug(f"Loading local image: {src}")
...
def _create_excel(data: list[list[str]], filename: str, folder_path: str | None = None, title: str | None = None) -> dict:
    ...
    if filename:
        # パス解釈脱出対策: エクスポートフォルダ外へ書き込まない（v1.0.60）
        filepath = _resolve_output_path(folder_path, filename)
...
def _create_pdf(text: str | list[str], filename: str, folder_path: str | None = None) -> dict:    
    ...
    def replace_image_query(match):
        ...
        return f'\n\n<img src="data:image/png;base64,{b64}" alt="Image: {query}" />\n\n'
...
def _create_presentation(slides_data: list[dict], filename: str, folder_path: str | None = None, title: str | None = None) -> dict:
    ...
    for i, slide_data in enumerate(slides_data):
        ...
        if image_query:
            image_data = search_image(image_query)
            if image_data:
                log.debug(f"Image generated for query: '{image_query}'")
        ...
        if isinstance(item, str):
            ...
            if item.get("type") == "image_query":
                ...
                if image_query:
                    ...
                    image_data = search_image(image_query)
                    if image_data:
                        doc.add_picture(BytesIO(image_data), width=Inches(6))
...
def _add_native_pptx_comment_zip(pptx_path, slide_num, comment_text, author_id, x=100, y=100):
    ...
    """
    # パス解釈脱出対策: スライド番号は必ず整数に限定する（v1.0.60）
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if authors_file.exists():
            ...
            if rels_file.exists():
                ...
                Path(rels_file).write_bytes(etree.tostring(rels_root, xml_declaration=True, encoding='UTF-8', pretty_print=True))
        
        Path(authors_file).write_bytes(etree.tostring(root, xml_declaration=True, encoding='UTF-8', pretty_print=True))
        ...
        if comment_file.exists():
            ...
            Path(slide_rels_file).write_bytes(etree.tostring(slide_rels_root, xml_declaration=True, encoding='UTF-8', pretty_print=True))
        ...
        Path(comment_file).write_bytes(etree.tostring(comments_root, xml_declaration=True, encoding='UTF-8', pretty_print=True))
        ...
        if content_types_file.exists():
            ...
            Path(content_types_file).write_bytes(etree.tostring(ct_root, xml_declaration=True, encoding='UTF-8', pretty_print=True))
...
async def review_document(
    ...
    try:
        ...
        if file_type == "docx":
            ...
            try:
                temp_pptx = Path(temp_folder) / "temp_input.pptx"
                temp_pptx.write_bytes(user_file.read())
