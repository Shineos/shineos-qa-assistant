...
public sealed class ChatFlow
{
    ...
    public async Task RunAsync(HttpContext ctx)
    {
        ...
        try
        {
            ...
            // 意図不明な極端に短い質問（例: 「テスト」）: 「テスト用サンプル」など表層一致する無関係文書が
            // 検索にヒットしても回答を試みてしまい、無関係な事実の列挙になる実測がある。短クエリのときだけ
            // 確認促進の注記を添える（SystemPromptへの常時追加はquick 1.7Bが正常質問まで自己矛盾したため撤回）
            if (message.Trim().Length <= 3)
                ctxPart += "\n\n※質問が短すぎて意図を確認できません。参照情報が質問に答えていない場合は、無関係な情報を列挙せず「質問の意図が確認できません。知りたいことを具体的に教えてください」と返してください。";
