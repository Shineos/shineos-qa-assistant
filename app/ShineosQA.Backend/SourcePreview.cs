...
/// <summary>出典プレビュー（図面）: 元PDFの全ページを画像化し、回答の根拠スニペットが
/// 現れている領域を図上の矩形（画像幅高の百分率）で返す（複数ページ対応）。
...
public static class SourcePreview
{
    ...
    public sealed record PageShot(int Page, string ImageDataUrl, int Width, int Height, List<RectPct> Rects);

    public sealed record PreviewResult(List<PageShot> Pages, int HitPage);
    ...
    public static async Task<PreviewResult?> BuildAsync(byte[] pdfBytes, string snippet, CancellationToken ct)
    {
        ...
        // 該当行（スニペット範囲と重なる行）を収集し、多数決でハイライトページを決める
        var hitLines = idx >= 0
            ? ex.Lines.Where(l => l.End > idx && l.Start < idx + matchLen).ToList()
            : new List<PdfLineInfo>();
        int hitPage = hitLines.Count > 0
            ? hitLines.GroupBy(l => l.Page).OrderByDescending(g => g.Count()).First().Key
            : DrawingIngest.PickSheetPage(ex.Runs, ex.Pages);

        // 全ページを WinRT（Windows.Data.Pdf）で描画。DocnetはスキャンCMYK-JPEGを黒つぶしするため使わない
        var pages = new List<PageShot>();
        for (int p = 1; p <= ex.Pages; p++)
        {
            var (png, imgW, imgH) = await Ingest.RenderPageWinRtAsync(pdfBytes, p - 1, 1400, ct);
            var rects = new List<RectPct>();
            if (p == hitPage && hitLines.Count > 0)
            {
                var (tpw, tph) = p - 1 < ex.PageSizes.Count ? ex.PageSizes[p - 1] : (0f, 0f);
                if (tpw > 0 && tph > 0)
                {
            ...
            pages.Add(new PageShot(p, "data:image/png;base64," + Convert.ToBase64String(png), imgW, imgH, rects));
        }

        return new PreviewResult(pages, hitPage);
    }
}
